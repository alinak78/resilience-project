...
# Use only the 'Description' column
tech_descriptions = df['Technologies'].dropna().tolist()
...
descr_texts = [preprocess_text_with_noun_noun_bigrams(text) for text in descriptions]

# Flatten the bigrams for frequency analysis
all_bigrams = [bigram for text in texts for bigram in text]

all_tech_bigrams = [bigram for text in texts for bigram in text]
...
lda_gensim_tech, corpus_tech, dictionary_tech = gensim_lda(descr_texts)

# Save Gensim LDA visualization as HTML
lda_display = pyLDAvis.gensim.prepare(lda_gensim, corpus, dictionary)
lda_display_tech = pyLDAvis.gensim.prepare(lda_gensim_tech, corpus_tech, dictionary_tech)

pyLDAvis_html = pyLDAvis.prepared_data_to_html(lda_display)
pyLDAvis_html_tech = pyLDAvis.prepared_data_to_html(lda_display_tech)
...
prevalent_bigrams_tech = get_prevalent_noun_noun_bigrams_with_saliency(lda_gensim_tech, dictionary_tech, num_words=15, saliency_threshold=0.3)
wordclouds_html_tech = generate_html_wordclouds_bigrams(prevalent_bigrams_tech, grid_columns=3)
...
<body>
    {pyLDAvis_html_tech}
    <hr>
    {wordclouds_html_tech}
</body>
</html>
"""

# Save the combined HTML file
output_file = "lda_visualization_with_filtered_noun_bigram_wordclouds.html"
with open(output_file, "w") as f:
    f.write(final_html)

# Save the combined HTML file
output_file_tech = "lda_visualization_with_filtered_noun_tech_bigram_wordclouds.html"
with open(output_file_tech, "w") as f:
    f.write(final_html_tech)

# Analyze most common bigrams for each category
description_bigram_counts = Counter(all_bigrams)
tech_bigram_counts = Counter(all_tech_bigrams)


# Save bigrams to JSON files
with open("description_bigrams.json", "w") as desc_file:
    json.dump(description_bigram_counts.most_common(), desc_file, indent=4)

with open("technology_bigrams.json", "w") as tech_file:
    json.dump(tech_bigram_counts.most_common(), tech_file, indent=4)

print(f"Final visualization saved as '{output_file}'")

print(f"Final visualization saved as '{output_file_tech}'")
